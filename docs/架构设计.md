# Echo 架构设计

> 版本：v0.1 ｜ 更新日期：2026-09-11 ｜ 对应代码：`src/echo/`

---

## 1. 架构总览

```mermaid
flowchart TB
    subgraph 接入层
        CLI[main.py CLI 入口]
        WS[WebSocket 服务端 M3 规划中]
    end
    subgraph 编排层
        PIPE[ConversationPipeline 编排 + 打断 + 上下文]
        CTX[ServiceContext 依赖注入容器]
    end
    subgraph 能力层
        VAD[VADInterface Silero / Mock]
        ASR[ASRInterface faster-whisper / sherpa-onnx / Mock]
        LLM[LLMInterface Ollama / OpenAI 兼容 / Mock]
        TTS[TTSInterface edge-tts / Mock]
    end
    subgraph 大脑层
        AGENT[Agent 工具调用循环 + 记忆 + 情绪]
        TOOLS[ToolRegistry 时间 / 天气 / 记忆]
        FACTS[FactStore 长期记忆]
    end
    subgraph 基础设施
        AUDIO[audio/io.py 采集 + 播放]
        MEM[memory/chat_history.py 上下文 + 持久化]
        CONF[config.py Pydantic + conf.yaml]
    end
    CLI --> PIPE
    WS --> PIPE
    PIPE --> CTX
    CTX --> VAD
    CTX --> ASR
    CTX --> LLM
    CTX --> TTS
    PIPE --> AGENT
    AGENT --> LLM
    AGENT --> TOOLS
    AGENT --> FACTS
    PIPE --> AUDIO
    PIPE --> MEM
    CONF --> CTX
```

**分层原则**：上层只依赖下层接口；能力层四个组件互不依赖；基础设施不含业务判断。

---

## 2. 模块职责

| 模块 | 职责 | 不负责 |
|------|------|--------|
| `config.py` | 定义并校验全部配置（Pydantic） | 不创建任何组件 |
| `service_context.py` | 按配置创建并持有四个组件 | 不做业务编排 |
| `vad/` | 流式断句、说话状态、预缓冲 | 不做转写 |
| `asr/` | 音频 → 文本，含结果清洗 | 不管理对话 |
| `llm/` | 消息列表 → 回复文本 | 不管历史存储 |
| `tts/` | 文本 → 音频文件 | 不管播放设备 |
| `audio/io.py` | 麦克风采集、音频解码、可打断播放 | 不做 VAD 判断 |
| `memory/` | 多轮上下文、JSON 持久化、长期事实记忆 | 不做向量检索（M4） |
| `pipeline/` | 串链路、事件回调、打断、上下文组装 | 不直接碰模型 API |
| `agent/` | 工具调用循环、情绪解析、事件发射 | 不做音频处理 |
| `memory/facts.py` | 长期记忆（事实条目 + 关键词检索） | 不做向量检索（后续） |

### Agent 事件流（M3 Live2D 的接口）

Agent 在处理一轮时向外发事件，前端据此驱动角色：

| 事件 | 载荷 | 用途 |
|------|------|------|
| `thinking` | `{"step": n}` | 角色进入"思考"状态（可放思考动画） |
| `tool_call` | `{"name", "arguments"}` | 展示"正在查资料"的提示 |
| `tool_result` | `{"name", "result"}` | 展示工具返回，便于面试演示与排错 |
| `speech` | `{"text", "tools"}` | 气泡文字 + 交给 TTS |
| `emotion` | `{"emotion"}` | **驱动 Live2D 表情**（neutral/happy/sad/angry/surprised/shy） |

---

## 3. 关键设计决策

| 决策 | 理由 | 代价 |
|------|------|------|
| 四组件统一"接口 + 注册表工厂 + 配置 + DI" | 换后端零改动；可 mock；可并行演进 | 多一层间接，初学成本高 |
| 全链路统一 16kHz / 单声道 / float32 | 语音模型都在 16k 上训练；接口常量优于层层传参 | 需要重采样；采样率错误不易察觉 |
| 每个组件都提供 mock 实现 | 测试不依赖模型/网络/声卡，CI 可跑 | mock 验证不了识别效果，需真实冒烟测试补偿 |
| 阻塞推理放线程池（`asyncio.to_thread`） | 不阻塞事件循环，音频回调不被拖死 | 线程池任务无法真正取消 |
| 打断用 `threading.Event` 而非取消协程 | 播放是阻塞 IO；取消 `await` 不会停止底层写入 | 需自行管理线程生命周期 |
| 历史用"按条数裁剪 + JSON" | 简单、可解释、可恢复 | 不是长期记忆（M3 用摘要/检索替代） |
| 设备降级放在运行期而非初始化 | CUDA 缺库报错发生在首次推理 | 首次请求多一次失败重试 |

---

## 4. 运行时数据流

### 4.1 正常一轮对话

```mermaid
sequenceDiagram
    participant U as 用户
    participant M as 麦克风
    participant V as VAD 状态机
    participant A as ASR
    participant L as LLM
    participant T as TTS
    participant S as 播放线程
    U->>M: 说话
    M->>V: 512 采样/块
    V->>V: 双阈值 + 防抖 + 预缓冲
    V->>A: 完整语音段 bytes
    A->>A: to_thread 转写
    A->>L: 用户文本 + 历史
    L->>T: 回复文本
    T->>S: 音频文件路径
    S->>U: 播放
```

### 4.2 打断时序

```mermaid
sequenceDiagram
    participant S as 播放线程
    participant M as 麦克风
    participant V as VAD
    participant P as 管线主循环
    S->>S: 正在播放回复
    M->>V: 用户插话的音频块
    V-->>P: is_speaking 为 True
    P->>S: stop_event.set
    S-->>S: 写音频块间隙退出
    P->>P: 继续累积新语音段并处理
```

---

## 5. 并发模型

| 执行体 | 承载任务 | 说明 |
|--------|---------|------|
| 事件循环 | 两条并发流水线（采集/VAD、应答）、LLM/TTS 网络等待 | 不执行 CPU 密集计算 |
| 线程池（`to_thread`） | ASR 转写、阻塞式 IO | 默认线程数约等于 CPU 核数；任务不可取消 |
| 麦克风回调线程 | 采集音频 → 队列 | 队列满时丢弃旧块，保证低延迟 |
| 播放线程 | 写音频到声卡 | 每块检查 `stop_event`，支持打断 |

**编排模型（2026-09-11 改造，参考 Pipecat 的处理器并发模型）**：
`ConversationPipeline.run()` 现在同时运行两条协程流水线——
`_capture_loop`（采麦克风 + VAD，产出语音段入队）与 `_respond_loop`（ASR → LLM → TTS → 播放）。
推理期间采集不中断，因此不会再因为"思考时停止读麦克风"而丢音频。
配合三道保护：单句最长时长强制切分、音频断流强制收尾并重启采集、丢块/丢段计数。

**双工模式 `duplex_mode`**：
`half`（默认）播放前停止采集、播完恢复，外放安全；`barge_in` 播放期间继续监听并允许插话（需耳机）；
`full` 完全并发（等接入 AEC 后开放）。

**边界原则**：事件循环内只做等待与编排，任何超过几毫秒的同步调用都下沉到线程池。
这也是"打断不能靠 `cancel()`"的根因——取消的只是等待，而不是线程里的推理。

---

## 6. 配置与依赖注入流程

```mermaid
flowchart LR
    Y[conf.yaml] --> R[原始字典]
    R --> C[EchoConfig 强类型配置]
    C --> SC[ServiceContext]
    SC --> E[四个组件实例]
    E --> P[ConversationPipeline]
```

- 配置非法在启动阶段即报错（例如 `provider="rocm"`、`compute_type="float8"` 直接拦下）
- `xxx_type: "none"` 表示显式禁用该组件，对应引擎保持 `None`

---

## 7. 扩展指南：新增一个后端

以"新增通义千问 API 后端"为例：

1. 新建 `src/echo/llm/qwen_llm.py`，实现 `LLMInterface.chat()`
2. 加装饰器 `@register_llm("qwen")`
3. 在 `llm_factory.py` 的 `_BACKEND_MODULES` 登记模块与依赖（用于延迟导入提示）
4. `conf.yaml` 增加配置段，并把 `llm_type` 改成 `"qwen"`

现有工厂、容器、管线代码零改动——这是架构解耦的验收方式。

---

## 8. 错误处理与降级

| 场景 | 行为 |
|------|------|
| 第三方包未安装 | 延迟导入抛 `ImportError`，附安装命令；mock 链路不受影响 |
| 模型文件缺失 | `FileNotFoundError` + 下载指引，不进入推理 |
| GPU 不可用 / CUDA 库缺失 | 捕获设备类异常 → CPU + int8 重建模型重试 → 打印降级日志 |
| 音频播放失败 | 触发 `error` 事件，管线继续监听，不终止会话 |
| 历史文件损坏 | 视为空历史，程序正常启动 |
| 语音段过短 | 丢弃（低于 `min_utterance_seconds`） |

详见 `docs/真实Bug笔记.md` 的 Bug #1。

---

## 9. 性能设计与优化路线

当前实测（详见 `benchmarks/results.md`）：ASR 5 秒音频约 1.5s（CPU int8）、
LLM 整句 0.6–0.9s（DeepSeek API）、TTS 合成约 2.7s（长回复）。

瓶颈与优化顺序（按 2026-09-11 实测重排，**TTS 比 LLM 慢得多，优先动它**）：

1. **TTS 整句合成（约 2.7s，最大瓶颈）** → 按标点分句合成，边合成边播放
2. **LLM 非流式（0.6–0.9s）** → 改流式，结合分句策略进一步压低首字延迟
3. **VAD 收尾等待** → 动态调整 `required_misses`，静音阈值自适应
4. **模型选型** → CPU/GPU、量化档位对比，形成选型表

---

## 10. 测试策略

| 层次 | 覆盖内容 | 依赖真实模型 |
|------|---------|-------------|
| 契约测试 | 接口不可实例化、方法语义、类型约定 | 否 |
| 单元测试 | 配置校验、工厂注册与拒绝、标签清洗、历史裁剪 | 否 |
| 集成测试 | 管线端到端（假麦克风 + 假播放器 + mock 组件） | 否 |
| 防回归测试 | 设备降级、超短语音丢弃、打断停止播放 | 否 |
| Agent 测试 | 工具注册/调用、工具调用循环、轮次上限兜底、情绪解析、记忆去重 | 否（脚本化假模型） |
| 冒烟测试 | 真实模型转写、真实 TTS 出声（手工执行并记录数字） | 是 |

---

## 11. 风险与技术债

| 项 | 影响 | 计划 |
|----|------|------|
| 无 AEC | 外放时可能自打断 | M4：评估 WebRTC AEC 或 Koala/Krisp 滤波器 |
| 非流式链路 | 首字延迟高；工具调用往返多（实测 3 次往返 ≈6.9s） | M4：分句流式 TTS → LLM 流式 → 工具并行 |
| 线程池不可取消 | 打断后计算资源仍被占用 | 记录为已知限制 |
| SenseVoice 未实测 | 少一条中文优化路径 | 视时间补齐 |
| 无监控与压测 | 无法量化回归 | 每次改动补 `benchmarks` 记录 |
