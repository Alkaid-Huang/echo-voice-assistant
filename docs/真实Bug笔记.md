...
## Bug #3：超长保护在音频断流时永远不触发（被单测抓出）

**发现时间**：2026-09-11（编排层并发化改造时）

**现象**

为治理"说话太久状态机挂死"，新增了"单句超过 `max_utterance_seconds` 强制切分"。
写完逻辑后测试却一直不通过。

**根因**

这段检查最初写在"收到音频块之后"的分支里。当麦克风流中断（`mic.get()` 返回 `None`）时，
循环走的是 `continue` 分支，**超长检查根本不会执行**——也就是说，恰恰在最容易卡死的场景（断流）下，
保护逻辑形同虚设。

**修复**

抽出 `_enforce_max_utterance()`，在**每一轮循环**都调用（无论有没有新音频）。
并把 `audio_idle_timeout` 的处置也串起来：断流时先强制收尾当前语音段（不丢用户刚说的话），再重启采集流。

**防回归**

`tests/test_llm_tts_pipeline.py::test_pipeline_forced_flush_on_max_duration`：
用"永远处于说话状态"的假 VAD 驱动管线，断言超时后确实切分并进入 ASR。

**面试可讲点**

"保护逻辑要覆盖异常路径本身"。只在正常路径上加超时，等于给最需要保护的场景漏了保护——
这类问题只有把"断流/无数据"也当成一等公民来测才能发现。

---

## Bug #4：批处理里的中文注释让 `run.bat` 每次启动都报错

**发现时间**：2026-09-11

**现象**

`run.bat` 能正常启动项目，但每次都会先打印一行：
`'找不到就回退到系统' is not recognized as an internal or external command`。

**根因**

批处理文件里的 `rem` 注释含中文，而 Windows 控制台默认用 GBK 解析 `.bat`；
UTF-8 编码的中文被错误解码后，注释里的片段被当成了命令执行。

**修复**

把 `.bat` / `.ps1` 里的注释统一改成 ASCII 英文；中文说明放进 README。

**面试可讲点**

跨平台/跨编码的"隐形地雷"：代码逻辑没错，但文件编码和运行环境不匹配就会出怪问题。
处理这类问题的正确顺序是——先定位是**编码**问题（把文件另存为 GBK 或改成 ASCII 是否消失），
而不是先怀疑逻辑。

---

## 事故记录 #1：真实 API Key 被误提交到公开仓库

**时间**：2026-09-11

**经过**

本地密钥文件 `.env` 已加入 `.gitignore`，但 `.env.example`（模板文件，本来就是给用户填空的）
被填入了真实 Key，随后一次 `git add -A` 把它一起提交并推送到公开仓库。

**处置**

1. 立即把 `.env.example` 恢复为占位符并推送（当前版本不再包含 Key）；
2. 提醒撤销/轮换已泄露的 Key（公开过的 Key 必须视为已泄露）；
3. 加入**密钥扫描脚本** `tools/check_secrets.py`：扫描受版本控制的文件，
   出现 `sk-` 开头的长串就报错并列出文件名；已写入项目规范，提交前必跑。

**教训**

密钥只能放 `.env`（已被忽略）；模板文件永远只留占位符；
凡是"会被 `git add -A` 扫到"的文件都要按"会被公开"来对待。
